...
//            );
    ...
    public function allCustomer()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $customers = $this->model_account_customer->getCustomersMod($filter_data);
            foreach($customers as $customer){
                $this->json['data']['customers'][] = array(
                    'name' => $customer['name'],
                    'date_added' => date('d/m/Y', strtotime($customer['date_added'])),
                    'image' => $this->urlPrefix . 'image/' . $customer['image']
                );
            }
            $total = $this->model_account_customer->getTotalCustomers($filter_data);
            $this->json['data']['total_Customers'] = (int)$total;
