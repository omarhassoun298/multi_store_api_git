...
class ControllerRestAccount extends RestController
{
    ...
    public function account(){
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $this->statusCode = 405;
            $this->allowedHeaders = array("GET", "PUT", "POST");
        }
    ...
    public function editImage($post){
        if (!$this->customer->isLogged()) {
            ...
            if (!empty($this->request->files['image']['name'])) {
                ...
                $directory = DIR_IMAGE.'catalog/customers/'.$this->customer->getId();
                ...
                if (is_file($this->request->files['image']['tmp_name'])) {
                    ...
                    if ($this->request->files['image']['error'] != UPLOAD_ERR_OK) {
                        $this->error[][] = $this->language->get('error_upload_' . $this->request->files['image']['error']);
                    }
                    ...
                    if (empty($this->error)) {
                        ...
                        if (move_uploaded_file($this->request->files['image']['tmp_name'], $destination)) {
                            $post['image'] = 'catalog/customers/'.$this->customer->getId().'/' . $filename;
                        } else {
                            $this->error[][] = 'Failed to uploaded iamge.';
                        }
                    }
                } else {
            ...
            if (empty($this->json["error"]) && !empty($post['image'])) {
        ...
        if (!$this->customer->isLogged()) {
            ...
            if ($this->validate($post)) {
                ...
                if(isset($post['telephone'])) {
                    if($post['telephone'] != $this->customer->getTelephone() && !empty($this->customer->getTelephone())) {
                        $this->session->data['telephone'] = $post['telephone'];
    ...
    protected function validate($post){
        ...
        if (isset($post['telephone'])) {
            if ((utf8_strlen($post['telephone']) < 3) || (utf8_strlen($post['telephone']) > 32)) {
                ...
                if($post['telephone'] != $this->customer->getTelephone()) {
        ...
        if(isset($post['nationality'])) {
            if (empty($post['nationality'])) {
                $this->error[] = "Nationality is required!";
            }
        }

        if(isset($post['address']['address_1'])) {
            if (empty($post['address']['address_1'])) {
                $this->error[] = "Street Address is required!";
            }
        }

        if(isset($post['address']['city'])) {
            if (empty($post['address']['city'])) {
                $this->error[] = "City is required!";
            }
        }

        if(isset($post['occupation'])) {
            if ((utf8_strlen($post['occupation']) < 3) || (utf8_strlen($post['occupation']) > 22)) {
                $this->error[] = "Occupation should be between 3 and 32 characters!";
            }
        }

        if(isset($post['company'])) {
            if ((utf8_strlen($post['company']) < 3) || (utf8_strlen($post['company']) > 12)) {
                $this->error[] = "Company should be between 3 and 32 characters!";
            }
        }

        if(isset($post['annual_income'])) {
            if (empty($post['annual_income'])) {
                $this->error[] = "Annual Income is required!";
            }
        }

        if(isset($post['employment_status'])) {
            if (empty($post['employment_status'])) {
                $this->error[] = "Employment Status is required!";
            }
        }

        if(isset($post['terms_and_conditions'])) {
            if (empty($post['terms_and_conditions'])) {
                $this->error[] = "You must agree on terms and conditions!";
            }
        }
