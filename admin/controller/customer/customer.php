...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['dob'])) {
			...
			$data['dob'] = $customer_info['dob'] != '0000-00-00' ? $customer_info['dob'] : '';
		} else {
			$data['dob'] = '';
		}
		...
		if (isset($this->request->post['terms_and_conditions'])) {
			$data['terms_and_conditions'] = $this->request->post['terms_and_conditions'];
		} elseif (!empty($customer_info)) {
			$data['terms_and_conditions'] = $customer_info['terms_and_conditions'];
		} else {
		...
		$data['nationalities'] = $this->config->get('module_setting_nationalities');

		$data['annual_incomes'] = ["0 - 20,000$", "20,000$ - 50,000$", "50,000$ - 100,000$", "100,000$ - 200,000$", "250,000$ - 500,000$", "500,000$+"];
		$data['employment_statuses'] = ["Employed", "Self Employed", "Student", "Unemployed"];
		...
		if (isset($this->request->post['annual_income'])) {
			$data['annual_income'] = $this->request->post['annual_income'];
		} elseif (!empty($customer_info)) {
			$data['annual_income'] = $customer_info['annual_income'];
		} else {
			$data['annual_income'] = '';
		}

		if (isset($this->request->post['employment_status'])) {
			$data['employment_status'] = $this->request->post['employment_status'];
		} elseif (!empty($customer_info)) {
			$data['employment_status'] = $customer_info['employment_status'];
		} else {
	...
	protected function validateForm() {
		...
		if (isset($this->request->post['dob'])) {
            ...
            if (!($date && $date->format($format) === $dob)) {
                $this->error['dob'] = "Invalid date format. Please use YYYY-MM-DD";
            }
		}
