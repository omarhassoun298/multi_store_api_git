...
class ControllerExtensionReportAdvSales extends Controller { 
	private $error = array();
	
	public function index() { 			
		$this->load->language('extension/report/adv_sales');

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "extension WHERE code = 'adv_reports_sales'");
		if (empty($query->num_rows)) {	
			$this->session->data['error_so'] = $this->language->get('error_installed');
			$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));		
		}
		
		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('extension/report/adv_sales');
		...
		if ($this->config->get('advso' . $this->user->getId() . '_settings_filters')) {
			$advso_settings_filters = $this->config->get('advso' . $this->user->getId() . '_settings_filters');
		} else {
			$advso_settings_filters = array();
		}
		...
		if ($this->config->get('advso' . $this->user->getId() . '_settings_all_columns')) {
			$advso_settings_all_columns = $this->config->get('advso' . $this->user->getId() . '_settings_all_columns');
		} else {
		...
		$data['order_statuses'] = $this->model_extension_report_adv_sales->getOrderStatuses(); 	
		...
		$data['stores'] = $this->model_extension_report_adv_sales->getOrderStores();
		...
		$data['currencies'] = $this->model_extension_report_adv_sales->getOrderCurrencies();
		...
		$data['taxes'] = $this->model_extension_report_adv_sales->getOrderTaxes();	
		...
		$data['tax_classes'] = $this->model_extension_report_adv_sales->getOrderTaxClasses();
		...
		$data['geo_zones'] = $this->model_extension_report_adv_sales->getOrderGeoZones();
		...
		$data['customer_groups'] = $this->model_extension_report_adv_sales->getOrderCustomerGroups();	
		...
		$data['payment_methods'] = $this->model_extension_report_adv_sales->getOrderPaymentMethods();	
		...
		$data['shipping_methods'] = $this->model_extension_report_adv_sales->getOrderShippingMethods();	
		...
		$data['categories'] = $this->model_extension_report_adv_sales->getProductsCategories(0);
		...
		$data['manufacturers'] = $this->model_extension_report_adv_sales->getProductsManufacturers(); 
		...
		if ($advso_settings_filters and (in_array('option', $advso_settings_filters))) {		
		$data['order_options'] = $this->model_extension_report_adv_sales->getProductOptions();
		...
		if ($advso_settings_filters and (in_array('attribute', $advso_settings_filters))) {		
		$data['attributes'] = $this->model_extension_report_adv_sales->getProductAttributes();
		...
		$data['locations'] = $this->model_extension_report_adv_sales->getProductLocations();
		...
		if ($advso_settings_filters and (in_array('coupon_name', $advso_settings_filters))) {		
		$data['coupon_names'] = $this->model_extension_report_adv_sales->getOrderCouponNames();
		...
		if ($advso_settings_filters and (in_array('campaign_name', $advso_settings_filters))) {		
		$data['campaign_names'] = $this->model_extension_report_adv_sales->getOrderCampaignNames();
		...
		if (isset($_GET['cron'])) {
			$cron_settings = $this->config->get('advso' . $this->user->getId() . 'cron_setting') !== null ? unserialize($this->config->get('advso' . $this->user->getId() . 'cron_setting')) : '';
		...
		$selected_load_save_reports = $this->config->get('advso' . $this->user->getId() . 'sr_load_save_report') !== null ? unserialize($this->config->get('advso' . $this->user->getId() . 'sr_load_save_report')) : '';
		...
		$selected_cron_settings = $this->config->get('advso' . $this->user->getId() . 'cron_setting') !== null ? unserialize($this->config->get('advso' . $this->user->getId() . 'cron_setting')) : '';
		...
		$results = $this->model_extension_report_adv_sales->getSales($filter_data);
		$totals = $filter_limit != '999999' ? $this->model_extension_report_adv_sales->getSalesTotal($filter_data) : FALSE;
		...
		if ($filter_details != 'all_details_products' && $filter_details != 'all_details_orders') {
		  ...
		  if ($filter_report == 'tax') {
			...
			foreach ($results as $totals) {
				...
				$data['refunds_total'] = $filter_report == 'currency' ? '' : $this->currency->format($refunds_total, $this->config->get('config_currency'));
			...
			$option_data = array();
			$options = $this->model_extension_report_adv_sales->getOrderOptions($result['order_product_id']);
			...
			// Taxes
			$tax_data = array();
			$taxes = $this->model_extension_report_adv_sales->getOrderTaxesDivided($result['order_id']);
			...
			$data['custom_fields'] = $this->model_extension_report_adv_sales->getCustomFieldsNames($filter_data);			
		...
		if (!isset($_GET['cron'])) {
			...
			file_put_contents('so_json_array.txt', json_encode($data['orders']));
		}
		...
		if ($this->config->get('advso' . $this->user->getId() . '_settings_filters')) {
			...
			$data['advso_settings_filters'] = array('store','currency','tax','tax_class','geo_zone','customer_group','customer_name','customer_email','customer_telephone','payment_company','payment_address','payment_city','payment_zone','payment_postcode','payment_country','payment_method','shipping_company','shipping_address','shipping_city','shipping_zone','shipping_postcode','shipping_country','shipping_method','category','manufacturer','product','sku','model');
		}
		...
		$data['initialise'] = '';
		 if (!$query->rows) {
			...
			$settings_data = array(
				'advso' . $this->user->getId() . '_settings_filters' 		=> array('store','currency','tax','tax_class','geo_zone','customer_group','customer_name','customer_email','customer_telephone','payment_company','payment_address','payment_city','payment_zone','payment_postcode','payment_country','payment_method','shipping_company','shipping_address','shipping_city','shipping_zone','shipping_postcode','shipping_country','shipping_method','category','manufacturer','product','sku','model'),
		...
		$pagination->url = $this->url->link('extension/report/adv_sales', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$data['report_link'] = $this->url->link('extension/report/adv_sales', 'user_token=' . $this->session->data['user_token'], true);
		$data['save_report_link'] = 'index.php?route=extension/report/adv_sales'. $url;
		...
		$data['url'] = $this->url->link('extension/report/adv_sales', 'user_token=' . $this->session->data['user_token'] . $url . '&page='. $page, true);
		...
		if (!isset($_GET['cron'])) {
			$this->response->setOutput($this->load->view('extension/report/adv_sales', $data));
		} else {	
			$export_data = array(
			...
			$cron_settings = $this->config->get('advso' . $user_id . 'cron_setting') !== null ? unserialize($this->config->get('advso' . $user_id . 'cron_setting')) : '';
			foreach ($cron_settings as $cron_setting) {
				if (isset($_GET['cron_id']) && ($_GET['cron_id'] == $cron_setting['cron_id'])) {
			...
			if ($report_type == 'export_no_details' && $export_type == 'export_xls') {
				...
				chdir($cwd);			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xls.inc.php');
				exit();
				...
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xlsx.inc.php');
				} else {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xlsx_php8.inc.php');
				}
				exit();			
			} elseif ($report_type == 'export_no_details' && $export_type == 'export_csv') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_csv.inc.php');
				exit();
			} elseif ($report_type == 'export_no_details' && $export_type == 'export_pdf') {
				require_once(DIR_SYSTEM . 'library/dompdf/autoload.inc.php');
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_pdf.inc.php');
				exit();
			} elseif ($report_type == 'export_no_details' && $export_type == 'export_html') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_html.inc.php');
				exit();	
				...
				require_once('Spreadsheet/Excel/Writer.php');
				chdir($cwd);			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xls_basic_details.inc.php');
				exit();
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_xlsx') {
				if (PHP_VERSION_ID < 80000) {			
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel.php');
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel/IOFactory.php');			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xlsx_basic_details.inc.php');
				} else {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xlsx_php8_basic_details.inc.php');
				}
				exit();	
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_csv') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_csv_basic_details.inc.php');
				exit();				
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_pdf') {
				require_once(DIR_SYSTEM . 'library/dompdf/autoload.inc.php');
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_pdf_basic_details.inc.php');	
				exit();
			} elseif ($report_type == 'export_basic_details' && $export_type == 'export_html') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_html_basic_details.inc.php');
				exit();	
				...
				require_once('Spreadsheet/Excel/Writer.php');
				chdir($cwd);			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xls_all_details.inc.php');
				exit();
			} elseif (($report_type == 'export_all_details_products' or $report_type == 'export_all_details_orders') && $export_type == 'export_xlsx') {
				if (PHP_VERSION_ID < 80000) {			
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel.php');
				require_once(DIR_SYSTEM . 'library/PHPExcel/Classes/PHPExcel/IOFactory.php');			
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xlsx_all_details.inc.php');
				} else {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_xlsx_php8_all_details.inc.php');
				}
				exit();	
			} elseif (($report_type == 'export_all_details_products' or $report_type == 'export_all_details_orders') && $export_type == 'export_csv') {
				include(DIR_APPLICATION . 'controller/extension/report/adv_reports/so_export_csv_all_details.inc.php');
				exit();			
			} else {
				exit();
			}		
	...
	public function customer_autocomplete() {
		...
		$results = $this->model_extension_report_adv_sales->getCustomerAutocomplete($filter_data);
	...
	public function product_autocomplete() {
		...
		$results = $this->model_extension_report_adv_sales->getProductAutocomplete($filter_data);
	...
	public function coupon_autocomplete() {
		...
		$results = $this->model_extension_report_adv_sales->getCouponAutocomplete($filter_data);
	...
	public function voucher_autocomplete() {
		...
		$results = $this->model_extension_report_adv_sales->getVoucherAutocomplete($filter_data);
	...
	public function campaign_autocomplete() {
		...
		$results = $this->model_extension_report_adv_sales->getCampaignAutocomplete($filter_data);
	...
	public function settings($filter_data = array()) {
		...
		if (!$json) {
			if (!$this->user->hasPermission('modify', 'extension/report/adv_sales')) {
	...
	public function cron($filter_data = array()) {
		...
		$crons = unserialize($this->config->get('advso' . $this->user->getId() . 'cron_setting'));
	...
	public function export($filter_data = array()) {
		$this->load->language('extension/report/adv_sales');
		$orders_data = json_decode(file_get_contents('so_json_array.txt'), true);
