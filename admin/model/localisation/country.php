...
class ModelLocalisationCountry extends Model {
	...
	public function getCountries($data = array()) {
		if ($data) {
			...
			if(isset($data['filter_iso_code_2']) && !empty($data['filter_iso_code_2'])){
				$sql .= " AND `iso_code_2` LIKE '" . $this->db->escape($data['filter_iso_code_2']) . "%'";
			}

			if(isset($data['filter_iso_code_3']) && !empty($data['filter_iso_code_3'])){
	...
	public function getTotalCountries($data = array()) {
		$sql="SELECT COUNT(*) AS total FROM " . DB_PREFIX . "country WHERE 1";
