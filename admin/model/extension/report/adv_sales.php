<?php
class ModelExtensionReportAdvSales extends Model {
	public function getSales($data = array()) {
		...
		$opt = '';
		if ($this->config->get('advso' . $this->user->getId() . '_settings_filters') and (in_array('option', $this->config->get('advso' . $this->user->getId() . '_settings_filters')))) {		
		...
		$atr = '';
		if ($this->config->get('advso' . $this->user->getId() . '_settings_filters') and (in_array('attribute', $this->config->get('advso' . $this->user->getId() . '_settings_filters')))) {		
		...
		$cpn = '';
		if ($this->config->get('advso' . $this->user->getId() . '_settings_filters') and (in_array('coupon_name', $this->config->get('advso' . $this->user->getId() . '_settings_filters')))) {		
		...
		$cgn = '';
		if ($this->config->get('advso' . $this->user->getId() . '_settings_filters') and (in_array('campaign_name', $this->config->get('advso' . $this->user->getId() . '_settings_filters')))) {		
		...
		if (!$this->config->get('advso' . $this->user->getId() . '_settings_mv_columns') or (in_array('mv_customers', $this->config->get('advso' . $this->user->getId() . '_settings_mv_columns')))) {		
			$sql .= " COUNT(DISTINCT CONCAT(o.lastname,o.firstname)) AS customers, ";
		} else {
			$sql .= " '' AS customers, ";	
		}
		...
		if (isset($data['filter_details']) && $data['filter_details'] == 'basic_details') {
			...
			if (in_array('pl_prod_option', $this->config->get('advso' . $this->user->getId() . '_settings_pl_columns'))) {		
			...
			if (in_array('pl_prod_attributes', $this->config->get('advso' . $this->user->getId() . '_settings_pl_columns'))) {		
		...
		if (isset($data['filter_report']) && $data['filter_report'] == 'coupon') {
			...
			$sql .= "FROM `" . DB_PREFIX . "order` o" . $date . $sdate . $osi . $order_id . $order_value . $store . $cur . $tax . $tclass . $geo_zone . $cgrp . $cust . $email . $tel . $ip . $pcomp . $paddr . $pcity . $pzone . $ppsc . $pcntr . $pmeth . $scomp . $saddr . $scity . $szone . $spsc . $scntr . $smeth . $cat . $manu . $sku . $prod . $mod . $opt . $atr . $loc . $cpn . $cpc . $gvc . $cgn . $cgc;
		}
		}
		...
		if (in_array('all_prod_option', $this->config->get('advso' . $this->user->getId() . '_settings_all_columns'))) {		
		...
		if (in_array('all_prod_attributes', $this->config->get('advso' . $this->user->getId() . '_settings_all_columns'))) {		
	...
	if (in_array('all_custom_fields', $this->config->get('advso' . $this->user->getId() . '_settings_all_columns'))) {	
